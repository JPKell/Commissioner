"""Typed refusals — the error hierarchy SpotCheck raises, per spec §7 and §13.
...
:func:`~spotcheck.policy.OrderedClassificationPolicy.evaluate` never raises for a deny — a deny is
data, recorded through the same path as an approval
...
__all__ = ["SpotCheckError", "StoreFailure"]


class SpotCheckError(SuiteError):
    """Base for every error this package raises.

    Nothing raises it directly; it exists so a caller can catch every SpotCheck refusal with one
    ``except`` without also catching unrelated suite errors.
    """

    code: ClassVar[str] = "SPOTCHECK_ERROR"


class StoreFailure(SpotCheckError):
    """A ledger could not record a decision (spec §13).

    Raised by ``EgressLedger.record`` implementations in Phase 2, never by
    :mod:`spotcheck.policy`. An unrecordable governance decision is not a decision that may
