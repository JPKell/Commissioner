"""SpotCheck — records an egress verdict; enforcing it is the caller's (ADR-0054).
...
SpotCheck not installed.

Phase 1 is pure: no I/O, no SQL, no logging, no environment. ``spotcheck.ledger``/``spotcheck.sql``
and the ``spotcheck[sql]`` extra arrive in Phase 2.
...
from spotcheck.__about__ import __version__
from spotcheck.errors import SpotCheckError, StoreFailure
from spotcheck.policy import EgressPolicy, OrderedClassificationPolicy
from spotcheck.types import EgressDecision, EgressRequest, EgressTarget, Verdict
